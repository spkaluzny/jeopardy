...
```{r plot03, fig.show='hold', fig.align='center'}
jeopardyData %>%
  mutate(PercentCorrect =
    (NumRight + DDRight) /
...
```{r plot04, fig.show='hold', fig.align='center'}
jeopardyData %>%
  mutate(NumAttempted = NumRight + NumWrong + DDWrong + DDRight,
...
```{r plot05, fig.show='hold', fig.align='center'}
jeopardyData %>%
  filter(!is.na(Gender)) %>%
  mutate(PercentCorrect =
    (NumRight + DDRight) /
...
```{r plot10, fig.show='hold', fig.align='center'}
filter(jeopardyData,
  PlayerId == "Ken Jennings.software engineer.Salt Lake City.Utah",
    NumTimesOnShow < 75) %>%
...
```{r plot13, fig.show='hold', fig.align='center'}
