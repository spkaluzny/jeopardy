...
The distribution of number of wins:
```{r}
jeopardyData %>%
    group_by(PlayerId) %>%
    summarise(NumberWins = max(NumTimesOnShow) - 1) %>%
    filter(NumberWins > 1 & NumberWins < 19) %>%
    ggplot(aes(x=NumberWins)) +
