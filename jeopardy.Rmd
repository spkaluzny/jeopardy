...
jeopardyData %>%
    ...
    mutate(PercentCorrect =
        (NumRight + DDRight) / (NumRight + NumWrong + DDWrong + DDRight) * 100,
        NumAttempted = NumRight + NumWrong + DDWrong + DDRight) %>%
    ggplot(aes(x=NumAttempted, y=PercentCorrect)) +
...
data.frame(State = names(CountPlayersByState),
    CountPlayersByState = c(CountPlayersByState)) %>%
    filter(State %in% c(state.name, "D.C."))
    ggplot(aes(x=CountPlayersByState,
        y=ordered(State, levels=State[order(CountPlayersByState)]))) +
