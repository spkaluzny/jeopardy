...
Most of the analysis was done with the the `dplyr` and `ggplot2` packages.
...
jeopardyData %>%
    mutate(NumAttempted = NumRight + NumWrong + DDWrong + DDRight,
        PercentCorrect = (NumRight + DDRight) /
            (NumRight + NumWrong + DDWrong + DDRight) * 100) %>%
    filter(PercentCorrect > 99) %>%
    ggplot(aes(x = as.factor(NumAttempted))) + geom_bar()
```

Most players who get all questions correct only attmempting a third or
less of the questions.
...
for Max1000.
It is no surprising that the spread of the FinalScorew values
