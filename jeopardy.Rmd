...
The `dplyr` and `ggplot2` package will be used for the analysis and are loaded here.
...
#### Final Jeopary Answers

Distribution of final answers correct in each game.
Counts those that had no change between Second Round Score and Final Score
got the final answer wrong.
```{r}
finalCorrect <- function(score2, scorefinal) {
    scoreDiff <- scorefinal - score2
    sum(scoreDiff > 0)
}
jeopardyData %>%
    group_by(Show) %>%
    summarise(NumFinalCorrect=finalCorrect(SecondRoundScore, FinalScore)) %>%
    with(cbind(NumFinalCorrect = 0:3,
        Count = table(NumFinalCorrect),
        Percent = round(table(NumFinalCorrect) /
            sum(table(NumFinalCorrect)) * 100, 2)))

# Method 2
jeopardyData %>%
    mutate(ScoreDiff = FinalScore - SecondRoundScore) %>%
    group_by(Show) %>%
    summarise(FinalCorrect = sum(ScoreDiff > 0)) %>%
    with(table(FinalCorrect))
```
