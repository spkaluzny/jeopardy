...
```
This is an analysis of data about Jeopardy games that are available at http://www.j-archive.com/.
...
round(sum(!is.na(jeopardyData$Gender)) / nrow(jeopardyData) * 100, 2)` percent.
...
Leaving out Jennings, the number of games per player:
```{r}
jeopardyData %>%
    group_by(PlayerId) %>%
...
### Final Winnings
Looking at the dollar winnings we need to look separately at games before the
dollar values doubled and after.
