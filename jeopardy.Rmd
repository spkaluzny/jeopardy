...
```{r echo=FALSE}
load("jeopardyData.Rds")
```

This is an analysis of data about Jeopardy games that is avilable at http://www.j-archive.com/.
The data includes information mainly about the players: names, home city, winnings, etc.
There is no information about the content of the games questions (answers). 
...
Changes made include fixing spelling of states,
New York city boroughs changed to New York,
"now living in ..." and "now stationed at ..."
changed to the listed location.
The `jcleanData` function for cleaning is listed in the appendix.
...
Analysis
---------
The final data has `r nrow(jeopardyData)` rows and `r ncol(jeopardyData)` columns.
