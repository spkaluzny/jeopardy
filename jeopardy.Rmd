...
If a player gives the correct answer to a cluse, that dollar amount
...
Due ot time limitations, the players may not get to all of the clues.
The number of of clues attempted in a game by a single player:
```{r}
jeopardyData %>%
    group_by(Show) %>%
...
The number of of clues attempted in a game by all three players:
```{r}
jeopardyData %>%
    group_by(Show) %>%
    mutate(NumAttempted = NumRight + NumWrong + DDWrong + DDRight) %>%
    summarise(NumAttemptedAll = sum(NumAttempted)) %>%
    with(summary(NumAttemptedAll))
```
...
Looking further at players on for many games:
```{r}
jeopardyData %>%
    group_by(PlayerId) %>%
    summarise(MaxNumGames = max(NumTimesOnShow)) %>%
    top_n(6, MaxNumGames)
    # filter(MaxNumGames > 9) %>%
    # with(table(MaxNumGames))
```
...
This is over all the games played.
...
an low number of players:

```{r fig.width=8, fig.height=8}
ggplot(data=stateDF, aes(x=Population, y=NPlayers)) + 
    geom_point() +
    geom_text(data=subset(stateDF, State %in% c("Texas", "Florida")),
        aes(Population, NPlayers, label=State, vjust=1))
```
