...
dfJeop02 %>%
    ...
    ggplot(aes(x=FinalWinnings)) + geom_histogram() + facet_grid(Gender ~ .)
# As a density plot
dfJeop02 %>%
    filter(IsWinner, !is.na(Gender)) %>%
    ggplot(aes(x=FinalWinnings)) + geom_density(alpha=0.5) + aes(fill = Gender)
dfJeop02 %>%
    filter(IsWinner, !is.na(Gender)) %>%
    ggplot(aes(x=FinalWinnings)) + geom_histogram() + facet_grid(Gender ~ .)
